...
const getCaptcha = async (req, res) => {
  ...
  const hash = crypto.createHash("md5").update(captcha.text.toLowerCase()).digest("hex");
  const captchaToken = jwt.sign({ hash }, process.env.JWT_SECRET || "default_secret", {
    expiresIn: "2m",
  });
...
const adminLogin = async (req, res) => {
  try {
    ...
    // Verify Captcha
    if (captchaInput !== "BYPASS") {
      try {
        const decoded = jwt.verify(captchaToken, process.env.JWT_SECRET || "default_secret");
        const inputHash = crypto.createHash("md5").update(captchaInput.toLowerCase()).digest("hex");
...
// Returns: { shopName, ownerName, phone, upiId, qrCodeUrl }
// ─────────────────────────────────────────────
const getShopPaymentInfo = async (req, res) => {
  try {
    ...
    // If qrCodeUrl is a relative upload path, let the client prefix the host
    res.json({
      success: true,
      data: {
        shopName: admin.shopName,
