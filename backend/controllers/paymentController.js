...
const submitPaymentScreenshot = async (req, res) => {
  try {
    ...
    // Build screenshot URL only if a file was uploaded
    let screenshotUrl = "";
    if (hasScreenshot) {
    ...
    // Find existing payment for this scheme/month — update if pending/overdue/rejected,
    // or create a new one
    ...
    if (payment) {
      if (payment.status === "paid") {
        return res.status(400).json({ success: false, message: "This month's payment is already verified & paid" });
      }
      ...
      // Get latest gold rate for record keeping (will recompute on approve)
      const goldRateDoc = await GoldRate.findOne().sort({ date: -1 });
      const rate = goldRateDoc ? goldRateDoc.ratePerGram : 0;
      const grams = rate ? parseFloat((scheme.monthlyAmount / rate).toFixed(4)) : 0;
