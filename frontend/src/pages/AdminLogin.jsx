...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .al-captcha-container { transition: all 0.2s; }
  .al-captcha-container:hover { border-color: #1A7FD4 !important; box-shadow: 0 4px 12px rgba(26,127,212,0.12); }
  .al-captcha-refresh { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important; }
  .al-captcha-refresh:hover { background: #E2E8F0 !important; transform: rotate(180deg); }
  .al-captcha-container svg { width: 100%; height: 100%; filter: contrast(1.1); }
`;
...
export default function AdminLogin({ onLogin, onSignup }) {
  ...
  const fetchCaptcha = async () => {
    try {
      ...
      if (data.success) {
        setCaptchaData({ svg: data.captcha, token: data.captchaToken });
      }
    } catch (err) {
  ...
  const validate = () => {
    ...
    if (!captchaInput) e.captcha = "CAPTCHA is required";
    return e;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/auth/admin/login`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password, captchaInput, captchaToken: captchaData.token }),
      });
      const data = await res.json();
      ...
      <div className="al-right">
        {view === "forgot" ? (
          ...
          <div className="al-form-card">
            ...
            {/* CAPTCHA SECTION */}
            <div className="al-field f2" style={{ marginTop: 12 }}>
              <label className="al-label">Captcha</label>
              <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 10 }}>
                <div
                  className="al-captcha-container"
              ...
              <div className="al-input-wrap">
                ...
                <input
                  type="text"
                  className={`al-input${errors.captcha ? " error" : ""}`}
                  placeholder="Enter the captcha above"
                  value={captchaInput}
                  onChange={e => { setCaptchaInput(e.target.value); setErrors(p => ({ ...p, captcha: "" })); }}
                />
              </div>
              {errors.captcha && <div className="al-error">⚠ {errors.captcha}</div>}
            </div>
