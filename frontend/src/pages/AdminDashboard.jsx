...
const getFileUrl = (filePath) => {
  ...
  if (clean.startsWith("uploads/")) return ADMIN_API + "/" + clean;
  return ADMIN_API + "/uploads/" + clean.split("/").pop();
};
...
function PaymentsPage({ goldRate }) {
  ...
  const [settings, setSettings] = useState({
    shopName: "", name: "", phone: "", upiId: "", bankName: "", qrCodeUrl: ""
  });
    ...
    try {
      ...
      if (data.success && data.data) {
        setSettings({
          ...
          upiPayeeName: data.data.upiPayeeName || "",
  ...
  const executeSaveSettings = async (verificationToken) => {
    ...
    try {
      ...
      Object.keys(settings).forEach(k => {
        if (k !== "qrCodeUrl" && settings[k] !== null && settings[k] !== undefined) {
          fd.append(k, settings[k]);
        }
      });
      if (qrFile) fd.append("qrCode", qrFile);
      else fd.append("qrCodeUrl", settings.qrCodeUrl || "");
            ...
            {loading ? <div style={{ padding: 40, textAlign: "center", color: "var(--text-light)" }}>Loading settings...</div> : (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20 }}>
                  ...
                  <Input label="Bank & Branch (e.g. Axis - 7317)" value={settings.bankName} onChange={e => setSettings(s => ({ ...s, bankName: e.target.value }))} />
                  <Input label="Shop UPI ID" value={settings.upiId} onChange={e => setSettings(s => ({ ...s, upiId: e.target.value }))} />
                  <Input label="UPI Display Name (Payee)" value={settings.upiPayeeName || ""} onChange={e => setSettings(s => ({ ...s, upiPayeeName: e.target.value }))} />
                </div>

                <div style={{ marginTop: 24 }}>
                  <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "var(--text-light)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>Receiving QR Code Image</label>
                  <div style={{ display: "flex", gap: 20, alignItems: "center", background: "var(--bg-input)", padding: 16, borderRadius: 12, border: "1px solid var(--border-main)" }}>
                    {settings.qrCodeUrl && !qrFile ? (
                      <div style={{ textAlign: "center" }}>
                        <img src={getFileUrl(settings.qrCodeUrl)} alt="QR" style={{ width: 80, height: 80, borderRadius: 8, objectFit: "cover", border: "1.5px solid var(--border-alt)", background: "#fff", padding: 4 }} />
                        <div style={{ fontSize: 10, color: "var(--text-sub)", marginTop: 6 }}>Current active QR</div>
                      </div>
                    ) : (
                      <div style={{ width: 80, height: 80, borderRadius: 8, border: "1.5px dashed var(--text-light)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>📷</div>
                    )}
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, color: "var(--text-sub)", marginBottom: 12 }}>Upload a new QR Code image to replace the current one. Square images are highly recommended.</div>
                      <input type="file" accept="image/*" onChange={e => setQrFile(e.target.files[0])} style={{ width: "100%", padding: "10px", background: "var(--bg-page)", border: "1px solid var(--border-alt)", borderRadius: 8, color: "var(--text-main)", cursor: "pointer" }} />
                    </div>
                  </div>
                </div>
      ...
      <div className="sidebar-footer">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            {(adminInfo?.name || "Virat").charAt(0).toUpperCase()}
          </div>
          <div>
...
export default function AdminDashboard({ onLogout }) {
  ...
  const renderPage = () => {
    switch (active) {
      ...
      case "profile": return <ProfileAdminPage />;
